...
static func format_goal(progression_state: ProgressionState, upgrade_definitions: Array[UpgradeDefinition], condition_id := "", recent_route_memory := "") -> String:
	...
	if prepared_for_blackwater and recent_route_memory == "Glass Rim":
		return "Goal: revisit Glass Rim for timing, cargo, or the sealed rim promise if oxygen allows, then exit via Mirror/Wide/Hollow."
