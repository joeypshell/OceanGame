...
static func update_run_panel(host) -> void:
	...
	if host.dive_session.result == host.DiveSessionScript.Result.READY:
		...
		host.run_summary_label.text = host._format_run_summary(host._format_ready_panel_summary(), "ready")
	elif host.dive_session.result == host.DiveSessionScript.Result.EXTRACTED:
		host.run_panel.visible = true
		if host.surface_tab_index == host.SURFACE_TAB_NIGHT:
			host.run_title_label.text = host._format_expedition_day_title("Night")
			host.run_summary_label.text = host._format_run_summary(host._format_night_phase_summary(), "night")
		elif host.surface_tab_index == host.SURFACE_TAB_UPGRADES:
			host.run_title_label.text = "Surface Upgrade Bay"
			host.run_summary_label.text = host._format_run_summary("Banked:%s\n%s choose; %s buys.\n%s" % [
			...
			host.run_title_label.text = host._format_expedition_day_title("Result: Extraction")
			host.run_summary_label.text = host._format_run_summary(host.last_result_summary, "extracted")
		...
		if host.surface_tab_index == host.SURFACE_TAB_LOG:
			...
			host.run_title_label.text = host._format_expedition_day_title("Result: Failure")
			host.run_summary_label.text = host._format_run_summary(host.last_result_summary, "failed")
	else:
