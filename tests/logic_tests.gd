...
func _test_compact_dive_hud_helpers() -> void:
	...
	_expect(main.call("_cargo_slot_color", "empty").a <= 0.6, "empty cargo slots should stay translucent glass, not opaque blocks")
	...
	_expect(main.call("_tool_slot_color", "ready").a <= 0.65, "ready tool slots should stay translucent like reference HUD glass")
	_expect(main.call("_tool_slot_color", "locked").a <= 0.45, "locked tool slots should sit quieter than ready tools")
	_expect(main.call("_tool_slot_color", "active").a >= 0.7, "active tool slots should still read as selected")
