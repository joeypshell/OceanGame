...
func _initialize() -> void:
	...
	_run("cargo slot presenter", _test_cargo_slot_presenter)
...
func _test_cargo_slot_presenter() -> void:
	...
	for state in ["kelp_fiber", "shell_fragments", "glow_plankton", "scrap_metal", "driftwood", "quartz_glass"]:
		_expect(CargoSlotPresenterScript.cargo_slot_icon_polygon(state).size() > 0, "%s cargo slots should have a mini-icon polygon" % state)
	...
	_expect(CargoSlotPresenterScript.cargo_slot_icon_polygon("unknown_resource").is_empty(), "unknown cargo states should not show a resource mini-icon")
	...
	_expect(CargoSlotPresenterScript.cargo_slot_color("glow_plankton") == Color(0.38, 0.52, 0.12, 0.76), "glow cargo slot color should preserve the existing exact color")
	_expect(CargoSlotPresenterScript.cargo_slot_icon_color("shell_fragments") == Color(1.0, 0.96, 0.74, 0.95), "shell cargo icon color should preserve the existing exact color")
