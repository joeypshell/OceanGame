...
func _initialize() -> void:
	...
	_run("debug Hollow Reef payoff visual staging service", _test_hollow_reef_payoff_visual_staging_service)
	...
	_run("condition visual sync service", _test_condition_visual_sync_service)
...
func _test_debug_review_helpers() -> void:
	...
	_expect(main.visual_smoke_route_stage == "", "Lower Connector visual staging service should stay inert outside web visual smoke")
	BlueChimneyVisualStagingServiceScript.stage_pocket_visual_review(main)
	_expect(main.visual_smoke_route_stage == "", "Blue Chimney visual staging service should stay inert outside web visual smoke")
	BlackwaterVisualStagingServiceScript.stage_route_visual_review(main)
	_expect(main.visual_smoke_route_stage == "", "Blackwater visual staging service should stay inert outside web visual smoke")
	DuskTrenchVisualStagingServiceScript.stage_route_visual_review(main)
	_expect(main.visual_smoke_route_stage == "", "Dusk Trench visual staging service should stay inert outside web visual smoke")
...
func _test_ship_offload_visual_staging_service() -> void:
	var main := MainScene.instantiate()
	root.add_child(main)
	main.dive_session.reset(main.max_oxygen)
	...
	_expect(main.dive_session.result == DiveSessionScript.Result.DIVING, "Ship Offload staging should keep the daylight dive active")
	_expect(main.visual_smoke_route_stage == "ship_offload_complete", "Ship Offload staging should expose the deterministic review stage")
	_expect(main.dive_session.current_cargo.is_empty(), "Ship Offload staging should run the normal cargo offload path")
	_expect(main.progression_state.resource_count("driftwood") == 1, "Ship Offload staging should bank staged driftwood through progression")
...
func _test_hollow_reef_payoff_visual_staging_service() -> void:
	var main := MainScript.new()
	main.dive_session.reset(30.0)
	main.visual_smoke_route_stage = ""

	HollowReefVisualStagingServiceScript.stage_payoff_visual_review(main)
	_expect(main.dive_session.result == DiveSessionScript.Result.READY, "Hollow Reef payoff staging should stay inert outside web visual smoke")
	_expect(main.visual_smoke_route_stage == "", "Hollow Reef payoff staging should not set route state outside web visual smoke")
	_expect(not main.run_hollow_reef_reading_recovered, "Hollow Reef payoff staging should not recover reading state outside web visual smoke")
	main.free()
...
func _test_wreck_echo_route_first_pass() -> void:
	...
	_expect(payoff_halo.color.a > 0.3 and payoff_core.color.a > 0.8, "route presenter should show unrecovered payoff cores clearly")
	_expect(payoff_spark.visible and payoff_spark.color.a > 0.8, "route presenter should show unrecovered payoff sparks")
	RoutePresenterScript.sync_blackwater_trace_payoff(payoff_halo, payoff_core, payoff_spark, true)
	_expect(payoff_halo.color.a < 0.1 and payoff_core.color.a < 0.2, "route presenter should quiet recovered payoff cores")
	_expect(not payoff_spark.visible, "route presenter should hide recovered payoff sparks")
	RoutePresenterScript.sync_east_shelf_pocket_payoff(payoff_halo, payoff_core, payoff_spark, false)
	_expect(payoff_halo.color.a > 0.3 and payoff_core.color.g >= 1.0, "route presenter should show East Shelf payoff core brightly before recovery")
	RoutePresenterScript.sync_blue_chimney_payoff(payoff_halo, payoff_core, payoff_spark, true)
	_expect(payoff_halo.color.a < 0.1 and not payoff_spark.visible, "route presenter should quiet Blue Chimney payoff after recovery")
	...
	_expect(hatch_label.text == "ECHO LOCK", "route presenter should show sealed shelf echo lock before lens context")
	RoutePresenterScript.sync_sealed_shelf_hatch_state(hatch_shimmer, hatch_badge, hatch_label, hatch_bars, true, true)
	_expect(hatch_label.text == "OPEN" and hatch_badge.color.a > 0.7, "route presenter should show sealed shelf open state with key ownership")
	...
	_expect(not salvage_lane.visible and salvage_tool.text == "CUTTER NEEDED", "route presenter should keep salvage pocket sealed before cutter ownership")
	...
	_expect(salvage_lane.visible and salvage_tool.text == "CUTTER READY" and salvage_promise.text == "OPEN", "route presenter should show salvage pocket open state with cutter ownership")
	...
	_expect(gate_label.text == "RESONANCE SEAL" and not gate_sill.visible, "route presenter should show sealed Blackwater state before key path")
	...
	_expect(gate_label.text == "KEY READY" and gate_sill.visible and gate_return.color.a > 0.1, "route presenter should show Blackwater open state with resonance key")
	...
	_expect(pressure_boundary.monitoring and pressure_boundary.monitorable and pressure_label.text == "LOCKED", "route presenter should block pressure gate before seal ownership")
	...
	_expect(not pressure_boundary.monitoring and not pressure_boundary.monitorable and pressure_label.text == "OPEN", "route presenter should open pressure gate after seal ownership")
	_expect(pressure_badge.color.g > pressure_badge.color.r, "route presenter should keep open pressure gate green")
...
func _test_condition_visual_sync_service() -> void:
	var main := MainScript.new()
	...
	_expect(main.rare_signal_emphasis.visible, "Rare Signal should show the subtle signal emphasis")
	_expect(main.shelf_glimmer_opportunity.visible, "Rare Signal should show the shelf glimmer opportunity")
	_expect(main.blue_chimney_signal_opportunity.visible, "Rare Signal should show the Blue Chimney signal opportunity")
	_expect(main.route_choice_research_lane.color == Color(0.52, 0.96, 1.0, 0.28), "Rare Signal research lane color should stay exact")

	ConditionVisualSyncServiceScript.sync_dusk_trench_condition_nudge(main, "low_visibility")
	_expect(main.dusk_low_visibility_veil.color == Color(0.035, 0.055, 0.15, 0.32), "low-visibility Dusk veil color should stay exact")

	ConditionVisualSyncServiceScript.sync_wide_chamber_condition_nudge(main, "calm_current")
	_expect(main.wide_chamber_return_main.color == Color(0.66, 1.0, 0.88, 0.17), "calm-current Wide Reef return color should stay exact")

	ConditionVisualSyncServiceScript.sync_mirror_kelp_condition_nudge(main, "kelp_bloom")
	_expect(main.mirror_kelp_bloom_approach_wash.visible, "Kelp Bloom should show the Mirror Kelp approach wash")
	_expect(main.mirror_kelp_reflective_backwater.color == Color(0.08, 0.36, 0.42, 0.18), "Kelp Bloom backwater color should stay exact")

	ConditionVisualSyncServiceScript.sync_condition_visuals(main, "calm_current", false)
	_expect(not main.rare_signal_emphasis.visible, "non-Rare-Signal conditions should hide the signal emphasis")
	_expect(not main.shelf_glimmer_opportunity.visible, "non-Rare-Signal conditions should hide shelf glimmer")
	_expect(not main.blue_chimney_signal_opportunity.visible, "non-Rare-Signal conditions should hide Blue Chimney signal")
...
func _test_compact_dive_hud_helpers() -> void:
	...
	_expect(_control_rect(main_scene.active_stats_panel) == main_scene.ACTIVE_STATS_RECT, "HUD layout service should apply the active stats panel rect")
