...
Current active milestone: Hollow Reef Side Cave. Existing issues #554 through #558 should turn the first cave-like side branch into a readable compact route with one knowledge payoff, safe-return cue, deeper promise, deterministic visual evidence, and milestone validation. Keep it authored and compact: no full cave network, minimap, checklist, combat, harvesting, capture, monster-part economy, or broad procedural generation.
...
Hollow Reef implementation status: #552 added `HollowReefCave` as a visible compact side-cave mouth off Dusk Trench, with passive landmark metadata, dark cave-mouth shape, cool reef rim, threshold water, local reef teeth, a turnback lip, and an up-left return-current cue back toward the trench. #553 strengthened entrance/exit readability with a cool entrance lip, entrance ribs, an exit return ribbon, exit rib, and end/back-wall read so the branch does not feel like a one-way mistake. It deliberately does not add an interior system, interaction, resource pickup, predator, pressure boundary, collision, minimap/checklist behavior, combat, harvesting, or procedural cave generation.
