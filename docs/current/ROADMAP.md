...
Current ordered queue: Area 01 readability issues should run before #622. The immediate order is visual hierarchy contract, lower-route cluster declutter, label noise reduction, pickup/payoff/lock distinction, one targeted visual capture, then re-evaluate #622 and #623. `Salvage Cutter Return Payoff` issues #619-#621 are complete; #622 and #623 remain useful, but they should wait until the current route cluster can be read at normal play scale.
