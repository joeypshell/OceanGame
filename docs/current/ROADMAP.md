...
Current active milestone: Salvage Cutter Return Payoff. The next player-visible priority is to open one promised route/tool payoff in the existing Wide Reef Chamber cluster rather than adding another sealed branch or another readability pass. Use the salvage data cache, `CUTTER NEEDED` port, and sealed salvage pocket as the setup; keep the milestone narrow enough to prove that earlier dives change what the player can do later.

Current ordered queue: `Salvage Cutter Return Payoff` issues #619-#623. #619 defines `Salvage Cutter I` as a narrow upgrade path from salvage-cache evidence, #620 opens the sealed Wide Reef salvage pocket when the cutter is owned, #621 adds one salvage-pocket payoff with cargo-vs-knowledge pressure, #622 adds one light hazard/creature timing read if it changes the decision, and #623 adds focused guards/staging plus milestone validation. This queue is intentionally fewer/larger and player-visible; avoid broad crafting, salvage inventory, combat, minimap, quest log, surface management, or screenshot expansion.

Salvage Cutter Return Payoff status: #619 promotes the prior future cutter promise into `Salvage Cutter I`, a normal Upgrade Bay entry unlocked by extracted `Salvage Data Cache` evidence and existing resources (`Kelp Fiber x1`, `Shell Fragments x2`). #620 makes the existing Wide Reef salvage pocket visibly respond to owned cutter state: without the cutter it remains `SALVAGE CLOSED` / `CUTTER NEEDED`, and with the cutter it becomes `SALVAGE OPEN` / `CUTTER READY` with a small opened pocket shell and Hollow Reef return cue. The payoff remains narrow and deliberately avoids salvage inventory, new resources, crafting trees, multiple cutter tiers, combat, harvesting, minimap, quest-log ownership, or a full wreck interior. #621 should add one contained payoff inside that opened pocket next; do not add another sealed promise before this route/tool payoff becomes useful.
