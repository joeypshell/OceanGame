...
Salvage Cutter Return Payoff status: #619 promotes the prior future cutter promise into `Salvage Cutter I`, a normal Upgrade Bay entry unlocked by extracted `Salvage Data Cache` evidence and existing resources (`Kelp Fiber x1`, `Shell Fragments x2`). It keeps the payoff narrow to the sealed Wide Reef salvage pocket and deliberately avoids salvage inventory, new resources, crafting trees, multiple cutter tiers, combat, harvesting, minimap, or quest-log ownership. #620 should make the sealed pocket respond to owned cutter state next; do not add another sealed promise before this route/tool payoff becomes playable.
