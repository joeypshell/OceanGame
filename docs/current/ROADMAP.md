...
First lower connector plan: `docs/planning/LOWER_CONNECTOR_PLAN_2026_06_26.md` defines `Shelf Drop Connector` as the next narrow route extension beyond East Shelf. The first scaffold now runs downward-right from the East Shelf pocket area with a visible connector mouth, subdued drop shelves, faint down-current thread, `Drop Arch` landmark, up-left return-current cue, turnback hint, tight camera/world-space extension, and `Drop Echo` as one run-scoped research opportunity near the turnback point. It is not a new biome, full cave network, minimap, quest tracker, combat route, or procedural map layer.

Sealed Shelf Hatch promise: `docs/planning/SEALED_SHELF_HATCH_UPGRADE_PROMISE_2026_06_26.md` defines `Resonance Key I` as a future narrow hatch-specific access module, not a broad upgrade tier. The current hatch should remain closed; `Echo Lens I` makes it readable, and East Shelf or Drop Echo research can later justify lab planning for one small key/pocket without adding a minimap, quest checklist, field guide, interior system, or new economy now.
...
Status: the first East Shelf Spur prototype is implemented and covered by focused logic plus Playwright visual capture. The next issue queue should come from the post-expanded-region backlog refresh, not the older polish-heavy vertical-slice queue.
...
Current implemented signal: the prototype now has practical scan/cargo upgrades, `Gulper Eel` observation, `Predator Warning I`, `Decoy Pulse I`, `Echo Lens I`, surface result memory, compact surface views, clean-save reset, HUD mode separation, runtime compact markers, authored and reviewed `Shell Reef` and `Thermal Vent` route pockets, pressure wreck/cache follow-through, the first narrow `Wreck Echo Descent` pocket with one run-scoped research clue, a documented hybrid-ocean model, expedition-day condition data, first source/export sprites for the player, boat, resources, creatures, and major landmarks, reusable readability scenes, screenshot workflow, debug seed/condition helpers, asset source conventions, a playable vertical-slice acceptance checklist, and the first right-side `East Shelf Spur` branch with stable landmark, return-current cues, seeded rare-signal opportunity, locked hatch promise, pocket interaction, current-surge timing cue, reset coverage, and deterministic Playwright captures. The next implementation target is the planned `Shelf Drop Connector`, a short downward-right continuation that starts proving larger side-view levels through one concrete route extension rather than another opening-slice polish pass.

Latest direction update: the completed expanded-route prototype should be followed by a route-gameplay batch: turn the pocket entrance into a minimal interaction, add one reason to choose East Shelf over the central descent on a given expedition, introduce one branch hazard or timing decision, and keep testing cheap through targeted state assertions and named Playwright captures.
