...
First lower connector plan: `docs/planning/LOWER_CONNECTOR_PLAN_2026_06_26.md` defines `Shelf Drop Connector` as the next narrow route extension beyond East Shelf. It should run downward-right from the East Shelf pocket area, add one local landmark, one return cue, and one optional research/resource opportunity, then stop at a turnback point. It is not a new biome, full cave network, minimap, quest tracker, combat route, or procedural map layer.
