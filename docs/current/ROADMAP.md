...
Mirror Kelp Pass implementation status: #609 adds the first authored `MirrorKelpPass` branch scaffold off `WideReefChamber`, with passive `LandmarkMetadata`, a dark branch mouth, reflective backwater, cool mirror-kelp curtains, floor/ceiling framing, a soft entry return ribbon, a stable prototype label, and tight player/camera/backdrop bounds. #610 adds a readable loop-return cue back through Wide Reef. #611 adds one run-scoped `Tideglass Sample` knowledge payoff inside the pass with visible recovered/available state, duplicate-safe interaction, compact extraction research memory, and no cargo, oxygen, durable route state, resource family, scan target, pressure, combat, minimap/checklist behavior, procedural cave system, or economy change. The next issues should add cargo-vs-knowledge choice and route pressure before any optional polish.
