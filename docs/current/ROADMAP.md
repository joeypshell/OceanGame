...
Hollow Reef interior lane: #568 adds the first short traversable lane beyond the Hollow Reef mouth and reading payoff, #569 adds a single optional upper-shelf choice inside that lane, #570 lets that shelf host existing Glow Plankton through the authored deep-reward spawn pool, and #571 adds `HollowReefSkitter` as a tiny passive scannable cave creature that teaches upper-shelf behavior through normal scan discovery/repeat-scan copy. These remain authored scene/placement changes with no new collision, interaction, resource type, extra pickup node, predator, pressure, save, map, quest, cargo-capacity, combat, harvesting, creature inventory, or procedural cave ownership; their purpose is to make the side-cave route feel physically entered, inhabited, and locally valuable during normal play before later small payoffs and the wider chamber milestone.
