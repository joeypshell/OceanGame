...
Lantern Ray implementation status: #546 added `LanternRayRoute` as the first non-combat lower-route creature presence near Dusk Trench, using a cool ambient lane and ray silhouette that stays visually distinct from red predator warnings and yellow/green resource cues. It is intentionally not a scan target, pickup, collision area, predator, interaction hotspot, cargo source, extraction blocker, or progression state yet. The next Lantern Ray issue should make it scannable as behavior research without adding combat or harvest behavior.
