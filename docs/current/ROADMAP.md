...
Current ordered queue: Area 01 readability issues should run before #622. #624 established the visual hierarchy contract and #625 quiets the Hollow Reef / Wide Reef / Mirror Kelp / salvage-pocket placeholder pileup so background terrain, return-current bands, opened-pocket support shapes, and condition nudges sit below the sub, pickups, knowledge payoffs, and lock reads. The remaining immediate order is label noise reduction, pickup/payoff/lock distinction, one targeted visual capture, then re-evaluate #622 and #623. `Salvage Cutter Return Payoff` issues #619-#621 are complete; #622 and #623 remain useful, but they should wait until the current route cluster can be read at normal play scale.
