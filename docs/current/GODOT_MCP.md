...
Adopt `hi-godot/godot-ai` as optional local development tooling for scene-aware Codex work. Do not vendor the addon into this repository unless a future issue explicitly changes that policy.
...
| Godot AI MCP | `addons/godot_ai/` when locally installed | Live Godot editor inspection and editor-assist tools | Yes | No, optional local install |
...
- Do not commit `addons/godot_ai/` unless a future issue explicitly decides to vendor the plugin.
...
2. Install Godot AI from the Godot Asset Library, Godot Asset Store, upstream release ZIP, or source checkout.
3. Open OceanGame in Godot.
4. Enable the plugin in Godot: Project Settings -> Plugins -> Godot AI.
5. Keep the Godot editor open with OceanGame loaded when using the MCP server.
6. Open the Godot AI dock and configure Codex, or add this to `~/.codex/config.toml`:
...
7. If telemetry is not desired, set `GODOT_AI_DISABLE_TELEMETRY=true` or `DISABLE_TELEMETRY=true` in the local environment before running the tool.
...
- Committing `addons/godot_ai/` or local MCP config just because the plugin was installed.
...
Adopted as optional local tooling. Setup requires local editor/plugin installation and local MCP client configuration, so this repository only documents the policy and setup shape. The plugin is not vendored in OceanGame.
