...
The Area 01 shell capture harness uses `tests/playwright/area01-capture-manifest.json` as its capture source of truth. Run `npm run test:area01-shell-captures` when Area 01 terrain, collision, cue ownership, gameplay-object visibility, or camera staging changes. It captures the current first-room shell states plus the developer-only source-map overlay and writes `area01-shell-capture-evidence.json` and `.md` beside the screenshots under `test-results/playwright-area01-shell/`. The report includes capture IDs, source-map revision, cue-family diagnostic counts/warnings from `window.__oceangameVisualState`, validation status for the capture assertions, resource/fish on-screen visibility summaries, change signatures for source-map/runtime/harness/docs files, a `visual_truth_gate_summary`, and a manual review checklist. The lower-route and future-exit capture states should stage against generated source-hook centers, such as the Blue Chimney return current and future deep-exit gate, so expected cue families stay tied to machine-readable source data rather than stale freehand camera coordinates. Treat those reports as review evidence, not golden-image infrastructure.

The Area 01 visual truth gate is intentionally stricter than state assertions. A passing Playwright run means the deterministic captures, metadata, player/resource/fish visibility assertions, and report were generated; it does not mean the cave walls are visually accepted. Before closing Area 01 visual/map issues, open or attach the generated screenshots and check the report's `visual_truth_gate_summary`. If `automated_gate_passed` is false, or if `blocker_signals` names cue warnings, missing expected cue families, missing expected resource silhouettes, or a missing expected Lantern Fry silhouette, do not close the visual/map issue until the blocker is resolved or explicitly carried by a follow-up issue. The manual review must also confirm that cave walls read as natural terrain rather than debug outlines, terrain fill/texture covers central and right-side walls, solid terrain separates from playable water, scan targets/resources are visually identifiable near prompts, fish silhouettes are visible where expected, and horizontal banding or flat placeholder polygons are absent from primary cave views.
...
The runtime builder may add budgeted, low-opacity visual seam dressing along generated source-grid water/solid boundaries, broken corridor/current eddies, plus organic merged water-skin overlays for player-facing readability. That decoration is reviewed by visual tests as a readability layer only; it must not become topology, collision, route, hook, or placement authority.
