...
- The player sub uses a distinct prototype silhouette with body, tail fins, dorsal/ventral fins, viewport, and nose light while retaining the same movement and collision behavior.
- The scene uses quieter surface light rays, subtle but readable depth transition bands, a safe-base return column with repeated return ribs, a compact return beacon near the surface lane, a more readable beaconed surface boat/lab and moonpool extraction origin, subdued shallow kelp silhouettes, a quieter midwater shelf and wreck silhouette, stronger Shell Reef landmark wash/shell silhouettes and return-current shapes, a warmer Thermal Vent landmark silhouette with chimney/mouth/plume cues, subdued cyan/green route-current hints, lighter deep pressure haze, a quieter deeper glow hint, firm blue pressure-gate rails/bars/shimmer that soften when open, a compact `LOCKED`/`OPEN` pressure badge, subdued pressure-wreck hull/cache signal glow, and red predator warning edge/rib/current/hazard shapes to communicate depth bands, safe return, blocked opportunity, and route danger without adding new mechanics. Safe-return shapes have higher visual priority than optional route and reward hints, and locked pressure routes are visually firmer than current hints.
...
- Source layout:
  ...
  - `scenes/Player.tscn`: prototype submersible scene.
