...
- Surface states are panel-led: ready/result/failure screens hide loose operational dive HUD rows such as oxygen, depth, base distance, cargo, scan target, discoveries, and status so they do not overlap the ready/result panels.
- Active dives show only the compact operational HUD: oxygen, depth, base direction, one-line cargo, compact discovery count, current scan target, short prompt, and bounded status text. Banked resources, upgrade details, recent expedition history, and full discovery lore remain out of the loose dive HUD.
