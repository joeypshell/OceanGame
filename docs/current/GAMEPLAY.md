...
- Dives begin from an `Expedition Ready` panel. Press E or Enter to begin oxygen drain and active dive play. The panel includes one compact `Today:` condition block that says why this expedition's ocean feels different without exposing seeds or implying unimplemented systems, plus one concise goal line that points to the next useful upgrade purchase, missing banked resources, scan prerequisite such as `Thermal Vent` or `Wreck Signal Cache`, or the `Shell Reef` midwater route objective once configured upgrade goals are complete.
- Extraction and oxygen failure show a run result panel summarizing banked cargo, carried-cargo loss, one concise route-choice callout, any compact Gulper research evidence, durable upgrade progress, discoveries recorded or kept, next-expedition framing, and best depth.
