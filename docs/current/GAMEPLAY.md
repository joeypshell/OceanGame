...
- `Echo Lens I` requires the `Wreck Signal Cache` discovery, ownership of `Signal Lens I`, and banked `Kelp Fiber x1`, `Shell Fragments x2`, and `Glow Plankton x3`. After ownership, repeat-scanning known `Wreck Signal Cache` returns one broad route curiosity line, `Echo Lens: weak wreck signal deeper-right.` This does not add minimap, field guide, exact-coordinate tracking, scanner energy, hidden object reveal, or objective checklist behavior.
