...
- Each expedition now selects a deterministic expedition condition from the current seed. The condition has an id, display name, short briefing line, and tags. `Thermal Bloom` currently adds a subtle non-mechanical warmth/bubble emphasis to the Thermal Vent visuals, and `Calm Current` subtly emphasizes safe-return lane/rib/beacon visuals; conditions do not yet change spawns, predator routes, oxygen, scan range, pressure gates, cargo, resources, movement, or upgrades.
