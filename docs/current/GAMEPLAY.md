...
- The surface base uses the clean continuous-hull generated research boat/lab sprite at `assets/sprites/base/research_boat_clean.png` under `SurfaceBaseArt/ResearchBoatSprite`. Earlier generated boat variants remain in `assets/sprites/base/` as source/reference assets, while the prior polygon boat, cabin, beacon, moonpool, moonpool-light, base-zone visual nodes, and oversized safe-return helper shapes remain hidden as fallback layout references; `BaseZone` remains the separate extraction/start gameplay area.
