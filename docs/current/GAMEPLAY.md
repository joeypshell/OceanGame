...
- The player sub uses a sprite-ready visual hierarchy under `VisualRoot`, with `SubSpriteAnchor/SubSprite` carrying the first generated PNG sprite at `assets/sprites/player/research_sub.png`. The earlier polygon hull, viewport, scanner mount, fins, cabin light, and `NoseLight` remain in the scene as hidden fallback placeholders, while `BubbleTrail` stays separate so future art/state work can replace or animate visuals without changing movement or collision behavior.
