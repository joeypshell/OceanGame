...
- Resource pickups use distinct prototype silhouettes: `Kelp Fiber` reads as green fronds/stem, `Shell Fragments` as pale ribbed shell shards, and `Glow Plankton` as a bioluminescent core/halo bloom. `Glow Plankton` now uses the first source/export resource asset at `assets/source/sprites/resource_glow_plankton_sprite_v1.svg` and `assets/exports/sprites/resource_glow_plankton_sprite_v1.svg`, with provenance in `assets/licenses/resource_glow_plankton_sprite_v1.txt`; its old polygon group remains hidden as fallback. These visual differences do not change pickup count, collision, scan behavior, or resource identity.
