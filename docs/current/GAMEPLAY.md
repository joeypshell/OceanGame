...
- Input is routed through Godot actions: `move_left`, `move_right`, `move_up`, `move_down`, `interact`, `restart_dive`, `scan`, and `burst_thruster`. F3 toggles development telemetry visibility. When development telemetry is visible, F4 cycles the current expedition condition for visual review and F5 advances the current review seed; these debug helpers are ignored while telemetry is hidden.
...
- The active dive HUD shows cargo as a compact count plus slot placeholders: filled slots tint by carried resource, empty slots show remaining capacity, and the fourth slot stays hidden/locked until `Cargo Rack I` raises capacity. This is a readability treatment only and does not change collection, extraction, failure, or banking behavior.
...
- The upgrade bay supports selecting configured upgrades with Up/Down while extracted. The current prototype has seven configured upgrades: `Oxygen Tank I`, `Pressure Seal I`, `Signal Lens I`, `Echo Lens I`, `Cargo Rack I`, `Predator Warning I`, and `Decoy Pulse I`.
...
- `Echo Lens I` requires the `Wreck Signal Cache` discovery, ownership of `Signal Lens I`, and banked `Kelp Fiber x1`, `Shell Fragments x2`, and `Glow Plankton x3`. After ownership, repeat-scanning known `Wreck Signal Cache` returns one broad route curiosity line, `Echo Lens: weak wreck signal deeper-right,` and briefly plays a local scan/echo pulse on the cache. This does not add minimap, field guide, exact-coordinate tracking, scanner energy, hidden object reveal, persistent marker, or objective checklist behavior.
...
- The HUD shows current depth, best depth reached, and base direction/distance so the return route remains understandable while diving. Oxygen uses normal, low, and critical player-facing states: low oxygen adds inline `LOW` text plus a compact upper-right warning panel, while critical oxygen adds inline `CRITICAL`, stronger color/pulse treatment, and return-to-base emphasis without changing oxygen costs, failure thresholds, upgrades, or save/load behavior.
- The player sub uses a sprite-ready visual hierarchy under `VisualRoot`, with `SubSpriteAnchor/SubSprite` carrying the first generated PNG sprite at `assets/sprites/player/research_sub.png`. The player physics root stays unmirrored during direction changes; only `VisualRoot` flips left/right so collision, camera, movement, and gameplay state remain stable. Idle and movement states are visual-only: `IdleBubble` appears while still, while `BubbleTrail` brightens and `ThrustFlare` appears while moving, including after left/right facing changes. The earlier polygon hull, viewport, scanner mount, fins, cabin light, and `NoseLight` remain in the scene as hidden fallback placeholders, while bubble/thrust visual nodes stay separate so future art/state work can replace or animate visuals without changing movement or collision behavior.
- The surface base uses the clean continuous-hull generated research boat/lab sprite at `assets/sprites/base/research_boat_clean.png` under `SurfaceBaseArt/SpriteAnchor/ResearchBoatSprite`. Earlier generated boat variants remain in `assets/sprites/base/` as source/reference assets, while the prior polygon boat, cabin, beacon, moonpool, moonpool-light, base-zone visual nodes, and oversized safe-return helper shapes remain hidden as fallback layout references under `SurfaceBaseArt/FallbackGeometry`; `BaseZone` remains the separate extraction/start gameplay area.
- The scene uses quieter surface light rays, subtle but readable depth transition bands, a more readable surface boat/lab extraction origin, subdued shallow kelp silhouettes, a quieter midwater shelf and wreck silhouette, stronger Shell Reef landmark wash/shell silhouettes and return-current shapes, a warmer Thermal Vent landmark silhouette with chimney/mouth/plume cues, subdued cyan/green route-current hints, lighter deep pressure haze, a quieter deeper glow hint, firm blue pressure-gate rails/bars/shimmer that soften when open, a compact `LOCKED`/`OPEN` pressure badge, subdued pressure-wreck hull/cache signal glow, and red predator warning edge/rib/current/hazard shapes to communicate depth bands, blocked opportunity, and route danger without adding new mechanics. The Shell Reef pocket, Thermal Vent pocket, and pressure wreck hull/gate visual clusters are instanced from `scenes/readability/`; gameplay ownership remains in `Main.tscn` through spawn candidates, scan targets, route reveals, the pressure boundary, upgrade state, and scan/cache nodes.
