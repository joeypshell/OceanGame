...
- `Echo Lens I` requires the `Wreck Signal Cache` discovery, ownership of `Signal Lens I`, and banked `Kelp Fiber x1`, `Shell Fragments x2`, and `Glow Plankton x3`. Its upgrade definition is present in the bay, but broad echo behavior is intentionally deferred to the next narrow issue. It should reveal only broad unresolved echo/category/direction hints and must not add minimap, field guide, exact-coordinate tracking, scanner energy, or objective checklist behavior.
