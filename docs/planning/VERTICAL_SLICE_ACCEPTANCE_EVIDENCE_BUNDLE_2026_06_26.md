...
Status: compact internal evidence bundle, refreshed through issue #393.
...
- Commit: `d5cf972`
- Review type: documented solo/internal evidence bundle plus state-asserted Playwright visual smoke
- Screenshot paths: local ignored artifacts under `test-results/playwright/`, with same-name `.json` metadata sidecars
...
- Screenshot/prompt hardening: `docs/planning/PLAYWRIGHT_SCREENSHOT_WORKFLOW_GAP_REVIEW_2026_06_26.md`, `docs/planning/PLAYER_ACTION_PROMPT_ABSTRACTION_PLAN_2026_06_26.md`, and `docs/planning/RESULT_UPGRADE_ACTIVE_COPY_REVIEW_AFTER_PROMPT_ABSTRACTION_2026_06_26.md`
...
Latest Playwright visual smoke passed with three scenarios and per-capture state assertions:

- surface ready, active dive, extraction result, upgrade tab, and lower-route pressure-gate views,
- staged Wreck Echo route, staged Wreck Echo result-readback, and no-debug Wreck Echo result player-facing views,
- active low oxygen and active critical oxygen HUD views.
...
- `test-results/playwright/visual-smoke-OceanGame-web-b76ec-e-and-result-readback-views-chromium/wreck-echo-result-player-facing.png`
- `test-results/playwright/visual-smoke-OceanGame-web-fec4e--critical-oxygen-HUD-states-chromium/active-low-oxygen.png`
- `test-results/playwright/visual-smoke-OceanGame-web-fec4e--critical-oxygen-HUD-states-chromium/active-critical-oxygen.png`

Each named screenshot has a matching `.json` sidecar recording expected state, actual exported visual state, and viewport size. This reduces the chance that a screenshot filename silently points to the wrong game state.

## Evidence Matrix

| Required Evidence | Current Status | Evidence | Watchlist |
| --- | --- | --- | --- |
| Ready screen | Pass with watchlist | Ready panel owns the keyboard start prompt, condition copy, and one goal line while active operational HUD rows and debug reset copy stay hidden by default. Prompt labels now route through a semantic helper without claiming controller/touch support. | Recheck surface panel contrast after final HUD polish. |
| Active dive | Pass with watchlist | Final compact HUD review plus Playwright low/critical captures confirm O2, depth, base, cargo slots, scan, prompt, status, low/critical warnings, result/failure hiding, and debug-on/off states are bounded at normal scale. | Fixed top-left HUD can cover incidental left-side art; keep future left-route art aware of that footprint. |
...
| Return/failure | Pass with watchlist | Extraction/failure result copy distinguishes carried cargo from banked/durable progress, keeps one memory line, and makes the next-expedition action clear. Durable progression reset/onboarding review found no save or copy change needed. | Maintain clean separation between durable progression and temporary dive state. |
| Upgrade view | Pass with watchlist | Upgrade bay is one selected upgrade at a time, shows keyboard selection/purchase prompts through the prompt helper, and keeps cost/state/missing requirements explicit after normal-scale review. | Upgrade details still risk lower-panel overflow; keep this as a narrow polish item and avoid tech-tree/inventory-grid expansion. |
...
| Wreck Echo route | Pass with watchlist | The first Wreck Echo pocket is implemented as one deeper-right optional research clue gated by Pressure Seal I plus Echo Lens I. Playwright staged route/result smoke and no-debug player-facing result capture pass after the first tuning pass. | Keep route small, run-scoped, no-locator, no-checklist, and no-combat; recheck lower-route density before any expansion. |
...
This bundle supports continued closeout, but the slice should stay `pass with watchlist` because lower-route density, source-art/fallback balance, upgrade detail overflow, mobile-like safe-area review, and future art changes still need rechecks before polished acceptance.
