# Roadmap History - Archived 2026-06-27

This is the full pre-compaction roadmap history. It is kept for completed issue batches, milestone history, and older planning context. The active roadmap now lives in `docs/current/ROADMAP.md`, and the navigation index lives in `docs/current/PROJECT_INDEX.md`.
...
North-star filter: current work should help the player see something tempting, risk a dive, return with cargo or knowledge, understand what changed, and want to try one smarter expedition tomorrow. A useful two-expedition check is whether the player can say, "I know what I want to try tomorrow." Prefer upgrades that open or clarify remembered places, daily variation that creates one meaningful route choice or surprise, surface results that explain what was learned, and creatures that communicate information, timing, danger, or misdirection before they become combat or loot. Avoid drifting into full open-world scale, co-op, restaurant/base management, exact scanner locators, broad crafting trees, minigame sprawl, or feature volume that weakens the expedition loop.

Latest milestone closeout: `docs/planning/WIDE_REEF_CHAMBER_MILESTONE_VALIDATION_2026_06_27.md` closes Wide Reef Chamber as pass with watchlist. Hollow Reef now opens into an authored side-to-side chamber with named landmarks, a Glassfin behavior read, salvage-cache payoff, optional cargo branch, future cutter promise, broad return-current language, result/recent-log memory, and debug staging. The chamber is real route-breadth progress, but it remains one branch cluster inside Area 01 rather than the full map.

Latest milestone closeout: `docs/planning/MIRROR_KELP_PASS_MILESTONE_VALIDATION_2026_06_27.md` validates Mirror Kelp Pass as pass with watchlist. The branch makes Area 01 feel wider through a named side route, Tideglass payoff, optional cargo pressure, Mirrorfin observation, Kelp Bloom approach variation, deep-kelp promise, and surface memory. The main remaining risk is promise buildup without enough payoff.

Current active milestone: Area 01 Readability Rescue. Recent playtesting says the Hollow Reef / Wide Reef / Mirror Kelp / salvage-pocket cluster is too messy to test comfortably, so readability is now a playability blocker rather than optional polish. Pause additional hazard/creature clutter until the current route is easier on the eyes. See `docs/planning/AREA_01_READABILITY_RESCUE_2026_06_27.md`.

Current ordered queue: Area 01 readability issues should run before #622. #624 established the visual hierarchy contract, #625 quiets the Hollow Reef / Wide Reef / Mirror Kelp / salvage-pocket placeholder pileup so background terrain, return-current bands, opened-pocket support shapes, and condition nudges sit below the sub, pickups, knowledge payoffs, and lock reads, #626 removes redundant normal-play route labels from the same cluster, #627 separates resources, knowledge payoffs, locked promises, and return/timing support into clearer color/opacity bands, and #628 records deterministic `wide_reef_salvage_open` evidence in `docs/planning/AREA_01_READABILITY_EVIDENCE_2026_06_27.md`. The readability evidence is pass-with-watchlist: #622 can resume, but its hazard/read must stay light and visually distinct. `Salvage Cutter Return Payoff` issues #619-#621 are complete; the remaining immediate order is #622, then #623 validation.

Salvage Cutter Return Payoff status: #619 promotes the prior future cutter promise into `Salvage Cutter I`, a normal Upgrade Bay entry unlocked by extracted `Salvage Data Cache` evidence and existing resources (`Kelp Fiber x1`, `Shell Fragments x2`). #620 makes the existing Wide Reef salvage pocket visibly respond to owned cutter state: without the cutter it remains `SALVAGE CLOSED` / `CUTTER NEEDED`, and with the cutter it becomes `SALVAGE OPEN` / `CUTTER READY` with a small opened pocket shell and Hollow Reef return cue. #621 adds a cutter-only `Salvage Manifest` payoff inside the opened pocket with cargo-vs-knowledge pressure against nearby existing Shell Fragments. The remaining #622 hazard/read and #623 validation are deferred until the Area 01 readability rescue makes the current route cluster testable.

Mirror Kelp Pass implementation status: #609 adds the first authored `MirrorKelpPass` branch scaffold off `WideReefChamber`, with passive `LandmarkMetadata`, a dark branch mouth, reflective backwater, cool mirror-kelp curtains, floor/ceiling framing, a soft entry return ribbon, a stable prototype label, and tight player/camera/backdrop bounds. #610 adds a readable loop-return cue back through Wide Reef. #611 adds one run-scoped `Tideglass Sample` knowledge payoff inside the pass with visible recovered/available state, duplicate-safe interaction, compact extraction research memory, and no cargo, oxygen, durable route state, resource family, scan target, pressure, combat, minimap/checklist behavior, procedural cave system, or economy change. #612 adds an optional deep-reward `Kelp Fiber` candidate below Tideglass so some expeditions make the player choose cargo, knowledge, both, or turning back while preserving current cargo limits, pickup ownership, extraction/failure behavior, and economy scope. #613 adds passive scannable `Mirrorfin Drift` behavior that moves across a reflection lane to teach safe-line timing without collision, damage, combat, drops, capture, field-guide objectives, or route-priority overrides. #614 lets the existing `Kelp Bloom` condition make the Mirror Kelp approach feel denser with a visible approach wash/rib, stronger reflective backwater, thicker kelp curtains, and compact ready/briefing copy while preserving stable geography, access, oxygen, cargo, progression, save behavior, and non-combat Mirrorfin/Tideglass rules. #615 adds `DeepKelpSealPromise` beyond the current Mirror Kelp payoff as a visual/readback-only sealed route mouth with kelp seal bars, cool pressure veil, future glint, and `DEEP KELP SEALED` copy; it points back to the remembered Mirror Kelp place without adding an upgrade, interaction, collision, pickup, save state, oxygen/cargo mutation, map marker, checklist, combat, harvesting, or interior. #616 lets Tideglass payoff or Mirrorfin observation surface as compact `Mirror Kelp Pass` route/result/recent-log memory, and lets prepared ready-panel copy nudge a revisit toward the deep-kelp seal while staying session-only and avoiding map/checklist/field-guide drift. #617 adds focused logic guards and deterministic Mirror Kelp debug staging for route, Tideglass, and Mirrorfin evidence; staging is debug/web-only, current-run scoped, and avoids durable progression mutation. #618 validates the milestone as pass with watchlist and selects `Salvage Cutter Return Payoff` as the next player-visible milestone.

Mirror Kelp Pass return-loop status: #610 strengthens the branch return read with a loop return-current ribbon, two return ribs, a quiet chamber-loop silhouette, and broad `RETURN VIA WIDE REEF` copy. This is scene/readability only: no teleport, route graph, exact locator, minimap, objective list, hidden oxygen tuning, collision, save data, or durable route state.

Recent completed milestone context: Hollow Reef Interior And Wide Chamber added route breadth before deeper monster-hunting or surface-progression systems became the focus. #568 establishes Hollow Reef as a short reachable interior lane, #569 adds the first optional upper-shelf choice inside it, #570 adds a sheltered existing-resource pocket, #571-#573 add small lane creature/content/memory, #574 opens the first authored wider chamber beyond it, #575 clarifies that chamber with far-wall, traversal-band, and foreground-edge silhouette language, #576 adds `Glass Rib Span` plus `Low Crown Shelf` as passive chamber landmarks, #577 adds a soft return-current chain across the wider chamber, #578 lets `Calm Current` subtly strengthen that chamber return chain without changing access, and #579 makes that chamber visible in normal expedition planning after prepared Hollow Reef route memory. `docs/planning/LOWER_ROUTE_CREATURE_PROGRESSION_VALIDATION_2026_06_27.md` validates Lantern Ray, Hollow Reef Skitter, and Glassfin Swarm as enough observation-first creature foundation for now: continue passive observation/avoidance later, but prioritize player-visible Area 01 breadth before adding deeper creature, salvage, or surface systems. Keep this focused on normal-play map growth and readable return planning: no minimap, checklist, route graph, broad quest log, combat, harvesting, capture, monster-part economy, broad field guide, or full procedural cave network.

Dusk Trench implementation status: #536 added the first scene-authored Dusk Trench continuation under `Blackwater Sill`, expanded the current camera/world/backdrop bounds for one compact lower-route step, and kept the route tied to existing Blackwater gate visibility without adding interaction, resources, combat, harvesting, minimap/checklist behavior, procedural generation, new upgrade family, or durable route state. #537 added passive Dusk Trench place metadata, broad return copy through Blackwater / Silt / Blue, and run-scoped reach memory for the result place line without adding durable route state. #538 added the calm `Glass Kelp Ledge` side pocket off Dusk Trench with ledge geometry, glass-kelp identity, and return-current readability. #539 added the single run-scoped `Glass Kelp reading` payoff with visible before/after state and extraction memory while keeping cargo, resources, combat, harvesting, minimap/checklist behavior, and durable route state out of scope. #540 added a dark blue-violet `LowVisibilityCue` with static murk/silt ribs to make the trench feel deeper while preserving return-current readability and avoiding oxygen, cargo, collision, predator, lock, or progression mutation. #541 made the existing `Low Visibility` condition a compact lower-trench nudge by strengthening the Dusk murk cue and ready/status copy without exact locators, mandatory route language, saved condition state, or gameplay mutation. #542 added Dusk reach priority to extraction route-choice memory and the compact recent-expedition log while keeping the evidence run-scoped and hidden from durable progression. #543 added deterministic Playwright captures for Dusk route, Glass Kelp payoff before recovery, and payoff recovered states using asserted web debug-command staging. #544 used that evidence for a local readability pass that strengthened Dusk return-current cues and offset/brightened the Glass Kelp payoff marker without changing oxygen, state ownership, objectives, combat, harvesting, or global route systems. #545 validated the milestone as pass with watchlist and selected Lantern Ray as the next player-visible gameplay milestone.

Lantern Ray implementation status: #546 added `LanternRayRoute` as the first non-combat lower-route creature presence near Dusk Trench, using a cool ambient lane and ray silhouette that stays visually distinct from red predator warnings and yellow/green resource cues. #547 made it a passive scannable behavior observation with compact first/repeat scan copy and durable normal discovery storage while keeping collision disabled and avoiding field-guide, monster-part, capture, combat, harvest, or objective-chain state. #548 added one pale timing/avoidance lane around the creature so the player can read a wait/pass/route-around window without hidden oxygen drain, cargo mutation, progression mutation, pressure behavior, damage, or harvest behavior. #549 added compact extraction/recent-log memory for Lantern Ray observation while keeping Dusk, Blackwater, Blue Chimney, Silt Vein, Shelf Drop, and East Shelf evidence higher priority for deeper route memory. #550 added deterministic authored Lantern Ray route variation through existing seed/condition route selection, including a Low Visibility-preferred high-safe glide, while keeping the selected route runtime/debug-only and out of durable save data. #551 validated the milestone as pass with watchlist and selected Hollow Reef Side Cave as the next map-growth milestone.

Hollow Reef implementation status: #552 added `HollowReefCave` as a visible compact side-cave mouth off Dusk Trench, with passive landmark metadata, dark cave-mouth shape, cool reef rim, threshold water, local reef teeth, a turnback lip, and an up-left return-current cue back toward the trench. #553 strengthened entrance/exit readability with a cool entrance lip, entrance ribs, an exit return ribbon, exit rib, and end/back-wall read so the branch does not feel like a one-way mistake. #554 added one run-scoped `Hollow Reef cave reading` payoff with visible before/after state, duplicate-safe interaction, extraction research memory, reset behavior, and no direct oxygen/cargo/progression mutation. #555 strengthened the local safe-return read with a pale return-chain, ribs, and landmark fleck pointing broadly back through Dusk Trench / Blackwater without adding movement, oxygen, cargo, progression, predator, lock, collision, interaction, map, checklist, exact route graph, or teleport behavior. #556 added `DeeperReefPromise` as a visible closed mouth beyond the current cave reading, with a subtle pressure wash, seal lip, glint, and `DEEP REEF CLOSED` label, while keeping it visual-only and out of durable save state. #557 added deterministic exported-browser visual captures for Hollow Reef entrance, payoff, and return-cue states through the existing Playwright debug-command bridge, with assertions on route stage and recovered reading state before each screenshot. #558 validates Hollow Reef as pass with watchlist and selects Lower-Route Expedition Goals as the next concrete milestone. #568 extends Hollow Reef with one short reachable `InteriorLane`, a lane back-water/shelf frame, a direction rib, and a lane return-current cue while modestly expanding the right-side player/camera bounds. #569 adds one small `UpperShelfChoice` above the main interior lane with cool shelf-pocket water, a shelf lip, an exploratory choice cue, and a softer drop-back cue to the safe lane. #570 adds a visual `ShelteredResourcePocket` in that upper shelf plus `StarterResourceCandidates/GlowPlankton/HollowReefA` as an optional deep-reward placement for the existing Glow Plankton pickup. #571 adds a tiny passive scannable `HollowReefSkitter`; #572 adds a low-alpha lavender `TimingCurrentCue` inside the interior lane with a deterministic wait/pass pulse; #573 lets either the cave reading or Skitter scan support compact Hollow Reef result/recent-log route memory without durable route state. Hollow Reef still does not add a full interior system, new resource type, extra pickup node, predator, pressure boundary, collision, knockback, minimap/checklist behavior, combat, harvesting, durable quest state, cargo-capacity change, or procedural cave generation.

Lower-route oxygen margin review: `docs/planning/LOWER_ROUTE_OXYGEN_MARGIN_REVIEW_2026_06_27.md` keeps current oxygen tuning after the Blackwater, Dusk Trench, Lantern Ray, and Hollow Reef route-growth passes. The direct lower-bound route remains technically viable, but the comfortable intended route is prepared play with `Oxygen Tank I`, clear turnback judgment, and local return cues. Future oxygen changes should require evidence from clean route, one-payoff, creature-pressure, or return-timing failures after local readability and placement fixes are tried.

Lower-route expedition goals validation: `docs/planning/LOWER_ROUTE_EXPEDITION_GOALS_VALIDATION_2026_06_27.md` selects route breadth as the next milestone over monster-hunting depth or surface progression. Monster hunting should wait until the larger route has space for passive creature behavior, and surface progression should wait until dives have more meaningful destinations. The open queue already contains the ordered follow-ups; do not create another broad batch from this validation.

Lower-route readability-scene polish: #564 extracts the Dusk Trench `LowVisibilityCue` into reusable `scenes/readability/DuskLowVisibilityCue.tscn` and lowers its neutral opacity so murk/silt pressure reads as atmosphere instead of another dense route pile. The cue remains visual-only under the existing `DuskTrench` owner and still has no interaction, collision, oxygen, cargo, pressure, predator, progression, or save ownership.

Lower-route source-art slot: #565 adds a matched source/export/provenance Hollow Reef cave landmark slot at `assets/source/sprites/landmark_hollow_reef_cave_sprite_v1.svg`, `assets/exports/sprites/landmark_hollow_reef_cave_sprite_v1.svg`, and `assets/licenses/landmark_hollow_reef_cave_sprite_v1.txt`. `HollowReefCave/SpriteAnchor/Sprite` documents those paths through metadata without requiring generated import metadata yet, while existing fallback geometry remains visible and gameplay ownership stays in the existing reading hotspot and route nodes.

Hollow Reef interior lane and first wide chamber: #568 adds the first short traversable lane beyond the Hollow Reef mouth and reading payoff, #569 adds a single optional upper-shelf choice inside that lane, #570 lets that shelf host existing Glow Plankton through the authored deep-reward spawn pool, #571 adds `HollowReefSkitter` as a tiny passive scannable cave creature that teaches upper-shelf behavior through normal scan discovery/repeat-scan copy, #572 adds a low-alpha lavender interior timing-current cue that pulses deterministically as a wait/pass read, #573 makes Hollow Reef observation/result memory session-only through the existing region, route-choice, and recent-log priority patterns, and #574 adds `WideReefChamber` as the first larger open traversal chamber beyond the lane. #575 gives that chamber a first readability silhouette pass with a cool far-wall silhouette, subtle traversal-clear band, and low foreground shelf edge so playable water reads separately from cave framing. #576 adds `Glass Rib Span` as an upper cool-rib landmark and `Low Crown Shelf` as a lower shelf landmark so the chamber can be remembered by places rather than coordinates. #577 extends the safe-return read with far, mid, and entry return-current cues that point broadly back through Hollow Reef without becoming objective arrows. #578 lets the existing `Calm Current` condition subtly strengthen those chamber return cues as deterministic presentation-only variation. #579 adds a compact ready-panel goal that points prepared players from recent Hollow Reef memory toward the wide chamber once upgrade/resource priorities are already clear. #580 starts the next creature-route layer with a passive `GlassfinSwarm` visual obstacle in the chamber that suggests spacing/timing while keeping the lower return route clear; #581 makes it a passive scannable behavior target through the existing scan/discovery system with compact first/repeat copy and no combat or collection ownership; #582 adds a deterministic pale spacing-window pulse that suggests when to pass without adding collision, damage, oxygen tax, cargo mutation, or tool requirements; #583 lets a Glassfin scan produce compact result and recent-log route memory while keeping higher-priority Hollow Reef/Dusk route evidence ahead and avoiding durable quest, journal, combat, or creature-inventory state; #585 adds a far-side `WreckSalvagePocketEntrance` as a closed salvage promise that stays out of the return path; #586 adds one run-scoped `Salvage data cache` payoff inside that entrance with visible recovered/available state and compact extraction memory; #587 adds an optional existing `Shell Fragments` deep-reward placement below-left of the cache so the far chamber can ask for cargo-vs-knowledge judgment without adding a new pickup node, salvage resource, loot table, or inventory system; #588 adds a warm visual-only `CUTTER NEEDED` port as the future salvage-tool promise while avoiding pressure, Echo Lens, Resonance Key, upgrade-definition, interaction, or collision behavior; #589 expands compact base-direction hints so Dusk and wide-chamber positions use `Dusk/Blackwater` or `Hollow/Dusk/Blackwater` return-chain language without adding minimap, breadcrumbs, exact locators, or checklist UI; #590 lets salvage-cache evidence remember `Wide Reef Chamber` in extraction route-choice copy and recent expeditions while preserving Hollow Reef, Dusk, Blackwater, Blue Chimney, Silt Vein, Shelf Drop, and East Shelf priority when that stronger chamber evidence is absent; #591 adds a visual-only `FarTurnbackCue` at the current far chamber edge with compact return-via-Hollow copy and a safe-current ribbon, without adding collision, interaction, route blocking, minimap, checklist, or exact locator behavior; #592 frames the run-scoped salvage cache as narrow future cutter prep in result/status/ready copy without adding a cutter upgrade definition, upgrade-bay entry, crafting, resource economy, save key, or purchasable tool. The chamber extends the camera/world/backdrop bounds only enough to support side-to-side movement, shelf framing, open-water negative space, a quiet future-choice shadow, a closed salvage promise with one knowledge payoff, one optional existing cargo branch, one future tool read, and a safe return-current cue back toward Hollow Reef. These remain authored scene/placement changes with no new route collision, resource type, extra pickup node, predator, pressure, save, map, quest, cargo-capacity, combat, harvesting, creature inventory, knockback, oxygen penalty, durable route state, salvage loot table, salvage inventory, crafting tree, cutter upgrade definition, wreck interior, minimap, breadcrumbs, or procedural cave ownership; their purpose is to make the side-cave route feel physically entered, inhabited, navigable, memorable, and locally valuable during normal play before later chamber content.
...
Wide chamber future-tool promise: #593 added compact Upgrade Bay planned/locked copy for the future `Salvage Cutter`; #619 supersedes that promise with the real, narrow `Salvage Cutter I` upgrade path. #620 makes the owned cutter visibly open the existing Wide Reef sealed pocket with a small return-readable lane, and #621 adds the first cutter-only `Salvage Manifest` payoff inside that lane with cargo-vs-knowledge pressure against nearby existing Shell Fragments. Keep remaining salvage work focused on readable local pocket payoff, return pressure, and one reason to revisit the route rather than broad crafting, salvage inventory, loot tables, a tool tree, combat, harvesting, or a wreck-interior system.
...
Current expanded-region implementation status: #446 widened the player world bounds, camera limits, and base ocean background bands so the next issues can place the first right-side branch without another camera/bounds pass. #447 added the first `East Shelf Spur` side-route scaffold using placeholder current, shelf, rib, gap, and terminal-pocket geometry. #448 added the first visible pocket entrance/readback point at the far end of the route without adding an interior system. #449 added `East Shelf Arch`, side-route return-current cues, landmark metadata, and horizontal base-direction copy such as `up-left`. #450 added `Shelf Glimmer`, a Rare Signal side-route visual opportunity near the arch. #451 added `Sealed Shelf Hatch`, an Echo Lens I-locked branch promise that changes presentation after scanner readiness without opening a new interior yet. #452 added reset/state-ownership coverage for the expanded route. #453 added a deterministic Playwright capture for the staged East Shelf route. #454 through #463 turned the branch into a minimal playable route with pocket interaction, run-scoped research payoff, extraction memory, branch goal fallback, current-surge timing cue, focused guard coverage, and a deterministic pocket-payoff Playwright capture.

First lower connector status: `docs/planning/LOWER_CONNECTOR_PLAN_2026_06_26.md` defined `Shelf Drop Connector` as the first narrow route extension beyond East Shelf, and the prototype is now implemented. The connector runs downward-right from the East Shelf pocket area with a visible connector mouth, subdued drop shelves, faint down-current thread, `Drop Arch` landmark, up-left return-current cue, turnback hint, tight camera/world-space extension, and `Drop Echo` as one run-scoped research opportunity near the turnback point. It is not a new biome, full cave network, minimap, quest tracker, combat route, or procedural map layer.
...
Roadmap refresh after East Shelf playable: `docs/planning/ROADMAP_REFRESH_AFTER_EAST_SHELF_PLAYABLE_2026_06_26.md` records that East Shelf now has interaction, payoff, result memory, timing/navigation behavior, and a first lower connector. The next batch should make the ocean feel bigger through concrete route behavior, not review-only polish.
...
- `docs/planning/LOWER_ROUTE_OXYGEN_MARGIN_REVIEW_2026_06_27.md`
- `docs/planning/LOWER_ROUTE_EXPEDITION_GOALS_VALIDATION_2026_06_27.md`
...
- `docs/planning/DEBUG_TELEMETRY_HIDDEN_STATE_SANITY_REVIEW_2026_06_26.md`
...
- `docs/planning/LOWER_ROUTE_RECHECK_AFTER_WRECK_ECHO_SOURCE_ART_2026_06_26.md`
- `docs/planning/RESOURCE_SCAN_AFFORDANCE_REVIEW_AFTER_WRECK_ECHO_2026_06_26.md`
- `docs/planning/RESULT_UPGRADE_COPY_REVIEW_AFTER_WRECK_ECHO_2026_06_26.md`
- `docs/planning/INPUT_ACTION_CONTRACT_REVIEW_2026_06_26.md`
- `docs/planning/FUTURE_CONTROLLER_SUPPORT_PLAN_2026_06_26.md`
- `docs/planning/FUTURE_MOBILE_LANDSCAPE_TOUCH_PLAN_2026_06_26.md`
- `docs/planning/PLAYWRIGHT_SCREENSHOT_WORKFLOW_GAP_REVIEW_2026_06_26.md`
...
Mobile-like low/critical oxygen coverage: `docs/planning/MOBILE_LIKE_LOW_CRITICAL_OXYGEN_CAPTURE_COVERAGE_2026_06_26.md` adds evidence-only Playwright captures for low and critical oxygen at `960x540`. This supports future touch safe-zone planning without implementing touch controls, controller support, mobile export settings, or HUD redesign.
...
Monster Research II state ownership: `docs/planning/MONSTER_RESEARCH_II_STATE_OWNERSHIP_RESULT_MEMORY_2026_06_26.md` keeps `Decoy response observed` run-scoped by default, allows one compact result `Research:` line, and defers durable progression state until a specific future upgrade or gate needs it. It should not create a field guide, checklist, minimap, route graph, creature inventory, combat log, or broad research tree.
...
Vertical-slice acceptance status: `docs/planning/PLAYABLE_VERTICAL_SLICE_ACCEPTANCE_CHECKLIST_2026_06_26.md` now reflects the latest compact HUD, pressure shell, lower-route, Decoy/Gulper, Wreck Echo planning, resource/scan, low-oxygen, upgrade, scanner-distinction, durable-progression, clean demo, surface-panel contrast, debug hidden-state, and MCP context refresh reviews. The slice is structurally playable and close to polished acceptance, with remaining pass-with-watchlist risk now centered on future visual-change regressions rather than known closeout blockers.
...
MCP context refresh after playable-loop polish: `.mcp/oceangame-context-server.mjs` and `docs/current/TOOLING.md` now expose the refreshed vertical-slice evidence bundle, clean demo script, surface contrast review, debug hidden-state review, pressure shell/lower-route evidence, compact HUD final review, Decoy/Gulper reviews, Wreck Echo planning, resource/scan review decisions, Wreck Echo no-implementation/data-shape/oxygen-cargo prep, closeout next-milestone planning, and the Wreck Echo prep backlog-health review.

Backlog replenishment after playable-loop closeout: `docs/planning/BACKLOG_REPLENISHMENT_AFTER_PLAYABLE_LOOP_CLOSEOUT_2026_06_26.md` moved the queue into `Wreck Echo Preparation And Slice Polish`. That preparation has now produced the first narrow Wreck Echo pocket; the preserved guardrails are still no locator, no combat, no broad biome, no new economy, and no durable quest/checklist state.

Next milestone after closeout: `docs/planning/NEXT_MILESTONE_AFTER_VERTICAL_SLICE_CLOSEOUT_2026_06_26.md` defined `Wreck Echo Preparation And Slice Polish`. The first route implementation has now landed, while broad Wreck Echo expansion, broad biomes, minimap/checklist UI, combat systems, and final art lock remain deferred.

Wreck Echo no-implementation criteria: `docs/planning/WRECK_ECHO_DESCENT_NO_IMPLEMENTATION_CRITERIA_2026_06_26.md` is now historical preparation guidance. The first implementation issue cleared one narrow route pocket and one run-scoped clue only; the broader blocked scope still applies to extra route nodes, scan-target sprawl, new resources, new pressure zones, persistent markers, exact locators, and scene-wired pressure-dark art.
...
Wreck Echo data shape: `docs/planning/WRECK_ECHO_DESCENT_ROUTE_POCKET_DATA_SHAPE_2026_06_26.md` guided the first route-pocket ownership split. The implemented pocket keeps authored placement in `Main.tscn`, uses run-scoped clue memory, and avoids durable Wreck Echo progression, new resources, markers, or pressure-dark art.

Wreck Echo oxygen/cargo preparation: `docs/planning/WRECK_ECHO_OXYGEN_CARGO_PREPARATION_REVIEW_2026_06_26.md` remains the active constraint for the implemented first pocket. Oxygen/return pressure carries the route decision, while cargo is not required because the first reward is research memory rather than a material/sample.

Wreck Echo readiness gate after shell/HUD polish: `docs/planning/WRECK_ECHO_READINESS_GATE_AFTER_SHELL_HUD_2026_06_26.md` is superseded for the first pocket only. The chosen research-clue reward, no-locator guardrails, and route-pocket planning allowed the narrow implementation; broader Wreck Echo expansion still needs fresh evidence.

Wreck Echo reward choice: `docs/planning/WRECK_ECHO_REWARD_CHOICE_2026_06_26.md` selected the compact research clue now used by the first Wreck Echo pocket. Rare material/prototype sample remains backup future scope; later prerequisite discovery remains rejected for the first slice because it risks quest-chain/checklist pressure.

Wreck Echo test coverage plan: `docs/planning/WRECK_ECHO_TEST_COVERAGE_PLAN_2026_06_26.md` now maps to implemented coverage for prerequisite gating, no-locator copy, compact research result memory, expedition reset behavior, scene contracts, and Playwright staged route/result evidence. Future Wreck Echo expansion should extend this coverage rather than broadening state silently.
...
Wreck Echo first implementation breakdown: `docs/planning/WRECK_ECHO_FIRST_IMPLEMENTATION_ISSUE_BREAKDOWN_2026_06_26.md` was completed by the first route pass: one authored pocket, one prepared-route question, one run-scoped research clue, focused tests, Playwright staged evidence, and strict no-locator/no-quest/no-new-economy non-goals.

Wreck Echo active HUD/prompt review: `docs/planning/WRECK_ECHO_ACTIVE_HUD_PROMPT_REVIEW_2026_06_26.md` applies to the implemented first pocket: Wreck Echo feedback stays inside existing compact `Scan`, prompt, status, oxygen, depth, base, and cargo HUD surfaces. Route panels, minimaps, active quest/objective UI, field guides, and new active upgrade/log surfaces remain rejected.

Wreck Echo lower-route capacity review: `docs/planning/WRECK_ECHO_LOWER_ROUTE_CAPACITY_REVIEW_2026_06_26.md` allowed the implemented single deeper-right pocket with a small footprint. Direct expansion inside the pressure gate/cache/reward/predator stack should still be avoided or deferred.

Wreck Echo result-memory guardrails: `docs/planning/WRECK_ECHO_RESULT_MEMORY_NO_QUEST_GUARDRAILS_2026_06_26.md` defines the implemented clue as one compact `Research:` result line and keeps attempt flags, route prompts, markers, objectives, field-guide entries, and checklist state out of durable progression by default.

Echo Lens to Wreck Echo handoff review: `docs/planning/ECHO_LENS_TO_WRECK_ECHO_HANDOFF_REVIEW_2026_06_26.md` keeps `Echo Lens I` as broad unresolved wreck curiosity and `Signal Lens I` as practical material scanning. Implemented and future Wreck Echo copy must not turn Echo Lens into an exact locator, objective marker, material detector, or pressure bypass.
...
Wreck Echo route-pocket ownership plan: `docs/planning/WRECK_ECHO_ROUTE_POCKET_OWNERSHIP_PLAN_2026_06_26.md` assigns future placement to authored scene nodes, orchestration/result formatting to `main.gd`, temporary attempt/clue state to `DiveSession` or a run helper, durable progression only to existing categories by default, and visuals to presentation-only readability scenes.
...
Surface launch/return review: `docs/planning/SURFACE_LAUNCH_RETURN_READABILITY_REVIEW_2026_06_26.md` keeps ready, launch, return/extract, and result surface states as pass-with-watchlist after moonpool and panel changes. Boat, moonpool, waterline, sub, and `BaseZone` read as one surface system while gameplay ownership stays separate.
